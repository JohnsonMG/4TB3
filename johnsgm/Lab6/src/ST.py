"""
Pascal0 Symbol Table, Emil Sekerinski, February 2017.
...
class Type:
    def __init__(self, tp):
        self.tp = tp
    def __str__(self):
        return 'type ' + str(getattr(self, 'name', '')) + indent(self.tp)
...
######### CLASS ENUM ADDED BY GAVIN JOHNSON #################################
class Enum:
    def __init__(self, elements, size):
        self.elements, self.size = elements, size
    def __str__(self):
        return ' enum ' + str([str(e) for e in self.elements])
#############################################################################
class Record:
    def __init__(self, fields):
        self.fields = fields
    def __str__(self):
        return 'record ' + str([str(f) for f in self.fields])
