...
def comment():
    write(indent)
    write("<span style=\"color:grey;\">{"+indent)
    getChar()
    while chr(0) != ch != '}': 
        if ch == '\n': writeln(); write(indent); getChar()
        elif ch == '\r': 
            getChar()
            if ch == '\n': writeln(); write(indent); getChar()
        elif ch == '\t': write(indent)
        else: write(ch); getChar()
    if ch == chr(0): mark('comment not terminated')
    write(indent+"}</span>"); getChar()
...
#def commentLine():
#    write(indent+"<span style=\"color:grey;\">//")
#    getChar()
#    while ch >= ' ': write(ch); getChar()
#    write("</span>");
